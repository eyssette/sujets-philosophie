<script>
	...
	function copyURL() {
		...
		additionalConditionsArray.forEach((condition, index) => {
		...
		navigator.clipboard.writeText(
			baseURL + "#" + hash + conditionsHashStringURL
		);
	}
	...
	$: {
		...
		if (conditionsHashString) {
			conditionsHashArray = conditionsHashString
				.split("C")
				.map((v) => parseInt(v));
		} else {
			conditionsHashArray = [];
		...
		additionalConditionsArray.forEach((condition, index) => {
			if (
				...
				conditionsHashArray.includes(index + 1)
			) {
				conditionChecked[index] = true;
...
{#each additionalConditionsArray as condition, i}
	<p>
		<label for="condition-{i}">{condition[0]} </label> :
		<input
			type="checkbox"
			id="condition-{i}"
			name="condition-{i}"
			bind:checked={conditionChecked[i]} />
	</p>
{/each}
